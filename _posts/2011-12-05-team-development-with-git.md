...
tags: [philosophy,git,'development-process']
---
...
2. Each developer has a clone of this repository that they use for
   ...
   means of collaboration with their peers (i.e.,. GitHub).
...
There is a bit of a mind hack going on here that I encourage you to
keep intact. That is the 'sacredness' of canonical. It should never
become something that a developer pushes too as part of his daily
...
they are all on the same network then they setup the [Git
Server](http://tumblr.intranation.com/post/766290565/how-set-up-your-own-private-git-server-linux)
...
two approaches, I prefer addressing my peer's repositories
...
the quality of code that wont matter until and if it makes it into
canonical.

How Code Gets Into Canonical
----------------------------

We don't impose tools or process on developers in their repo. They can
code using any process they would like, using any editor, compiler,
platform, etc. It doesn't matter in the least as long as the *output*
meets the team's standards. Code that exists in the developers repo is
kind of like Potential Energy. Potential Energy is impotent, has no
interaction with the world around it until it becomes Kinetic
Energy. Once that Potential Energy becomes kinetic, it has the
potential to change the world. The code in a developer's repository is
very similar. It has no impact on the world (i.e., the
...
Energy. Let's talk a bit about the process used to convert that
...
review/validate that code. Let's get started on the producer side.
...
* That the code is good, in the eyes of the code reviewer. Functions
  are not too long, modules are focused, etc. That the code follows normal practice for producing good code.
...
case, once the code is ready he makes an announcement letting the team
know that fact. Usually, this is in the form of patches sent to a
mailing list, or a GitHub pull request. The mechanism doesn't matter
so much as the fact that an intentional announcement with a short
description is made to the team working on the project.
...
do it is just let the code reviewer self-select. As long is it's not
...
matters a lot less than the fact that one is selected.
...
The canonical branch will become painful etc. So the Developer needs to do
...
* You have a reasonably competent engineer.
...
process comes into it is in the transition from the
...
One of the ways I have seen the process founder and become rigid is
with respect to a team that does not know git well. This process takes
quite a bit of comfort with Git to work. Your developers need to be
comfortable with local and remote repositories, rebasing, merging,
pushing and pulling from peers, signing off on commits, etc. If you
have new or incompetent developers you may be tempted to wrap all of
these steps in scripts that, apparently, take the need for knowledge
away from your developers. However, these scripts don't remove the
need for knowledge of git, all they do is delay that need
slightly. What they do is encourage the team to avoid learning git,
while at the same time spending an inordinate amount of time fixing
problems when the scripts inevitably break. At the same time, they
lock down and rigidify a process that should, by its nature, not be
rigid.
...
system is down, and that outage is costing the company a million dollars a
...
situation is high, and it's much cheaper timewise to catch those
